#TODO: This is quite possible at the limits of what is reasonable without having the backing of some testing and with that I'm absolved of any responsibility :)
#FWIW, I think we're probably past that point now :(

[CmdletBinding()]
param (
    [string]
    $repo = "staff-infrastructure-print-xwc",
    [int]
    $pr,
    [string]
    $token,
...
function Minimize-Comment {
    param (
        ...
        $environment,
        [string]
        $author = "staff-infrastructure-moj",
        [string]
        $matchingString
    )
    ...
    Write-Host "Post to GraphQL API for environment: $environment and stageName: $stageName."
    ...
    $body = "{`"query`":`"{`\n  repository(name: `\`"$repo`\`", owner: `\`"ministryofjustice`\`") {`\n    pullRequest(number: $pr) {`\n      comments(last: $pageSize) {`\n        edges {`\n          node {`\n            id`\n            isMinimized   `\n            body         `\n            author{`\n              login`\n            }`\n          }`\n        }`\n      }`\n    }`\n  }`\n}`",`"variables`":{}}"
...
function Get-PlanBody {
    ...
    if (Test-Path $inputFile) {
        ...
        if ($commentBody.Length -gt 65536) {
            ...
            $body = @{"body" = "Environment: $environment. Pipeline Stage: $stageName`nPlan has been truncated:`n``````$($commentBody.Substring(0, 65400))" }
        }
        else {
            $body = @{"body" = "Environment: $environment. Pipeline Stage: $stageName`n``````$commentBody" }
        }
    }
    else {
        $body = @{"body" = $("Environment: $environment. Pipeline Stage: $stageName had no plan`nSomething has gone wrong see: https://dev.azure.com/MoJ-OFFICIAL/Velocity-Landing-Zone/_build/results?buildId={0}&view=charleszipp.azure-pipelines-tasks-terraform.azure-pipelines-tasks-terraform-plan" -f $buildId) }
        Write-Host "The inputfile is empty, i.e. no plan so linking to terraformcli@0 task."
    }
...
function Add-GithubComment {
    param (
        ...
        $body,
        $environment,
        $matchingString = "Environment: $environment. Pipeline Stage: $stageName"
    )
...
if ($isPlan) {
    ...
    if ($exitCode -eq 2) {
        ...
        Add-GithubComment -repo $repo -pr $pr -token $token -stageName $stageName -uri $uri -body $body -environment $environment -matchingString $('(?im)^Environment: {0}. Pipeline Stage: {1}' -f $environment, $stageName)
    }
    else {
        Write-Host "Plan has no changes, will try to minimize old comments"
        Minimize-Comment -repo $repo -pr $pr -token $token -stageName $stageName -environment $environment -matchingString $('(?im)^Environment: {0}. Pipeline Stage: {1}' -f $environment, $stageName)
    }
}
elseif ($isScan) {
    ...
    if ($scan.Result) {
        $body = @{"body" = $("Environment: $environment. Security Scan of Stage: $stageName has failed a IaC security scan, Please look at https://dev.azure.com/MoJ-OFFICIAL/Velocity-Landing-Zone/_build/results?buildId={0}&view=ms.vss-test-web.build-test-results-tab" -f $buildId) }
    }

    if ( $scan.Error) {
        $body = @{"body" = $("Environment: $environment. Security Scan of Stage: $stageName has an error, Please look at https://dev.azure.com/MoJ-OFFICIAL/Velocity-Landing-Zone/_build/results?buildId={0}&view=ms.vss-test-web.build-test-results-tab" -f $buildId) }
    }

    if ($scan.Result -or $scan.Error) {
        Add-GithubComment -repo $repo -pr $pr -token $token -stageName $stageName -uri $uri -body $body -environment $environment -matchingString "Environment: $environment. Security Scan of Stage: $stageName has"
    }
    else {
        Write-Host "Will try to minimize old comments as there don't appear to be any changes"
        Minimize-Comment -repo $repo -pr $pr -token $token -stageName $stageName -environment $environment -matchingString "Environment: $environment. Security Scan of Stage: $stageName has"
    }
}
